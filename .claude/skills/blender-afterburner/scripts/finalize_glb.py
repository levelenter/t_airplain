"""Post-process a GLB from create_afterburner.py.
...
args=sys.argv[1:];path=Path(args[0]);clip_name='Afterburner_Loop';copy_to=None
...
for channel in clip['channels']:
 ...
 assert abs(times[0][0])<1e-5 and abs(times[-1][0]-duration)<1e-5
 same=all(abs(x-y)<1e-3 for x,y in zip(track[0],track[-1]))
 # a rotation may end at -q (same orientation); translation / scale (travelling streaks) must match exactly
 assert same or (channel['target']['path']=='rotation' and all(abs(x+y)<1e-3 for x,y in zip(track[0],track[-1]))),'loop not seamless'
...
for mesh in doc['meshes']:
 for primitive in mesh['primitives']:
  ...
  alpha=[r[3]/divisor for r in values(primitive['attributes']['COLOR_0'])];assert min(alpha)==0 and max(alpha)>.08  # soft haze/shimmer ribbons are faint by design
...
report={'bytes':len(output),'meshes':len(doc['meshes']),'animation':clip_name,'duration':duration,'channels':len(clip['channels']),'unlit_vertex_glow':'passed','seamless_loop':'passed'}
